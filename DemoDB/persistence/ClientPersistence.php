...
class ClientPersistence extends PdoAdapter {
    ...
    public function getAll(): array {
        ...
        foreach ($rows as $data) {
            $arrayClients[] = new Client($data["id_client"], $data["nom"], $data["email"],
                    $data["cognom"], $data["provincia"], $data["poblacio"]);
        }
        return $arrayClients;
    }
    ...
    public function getByIdent(int $id): Client {
        ...
        if (count($data) > 0) {
            ...
            return new Client($data[0]["id_client"], $data[0]["nom"], $data[0]["email"],
                    $data[0]["cognom"], $data[0]["provincia"], $data[0]["poblacio"]
            );
        } else {
